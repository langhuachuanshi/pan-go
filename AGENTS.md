...
- 版本号格式：`[前缀/]v0.<年2位><周2位>.<周内序号>`——开发期 major 固定 0（含义=不承诺兼容，破坏性变更直接升 minor/patch，无需动路径）；minor = 年2位×100+周2位（如 2637 = 26 年 37 周），跨年天然单调（2701 > 2637）；周内序号从 1 顺加。示例：`v0.2637.1`。
- 多模块仓库**只打模块 tag**（`quark/v0.2637.1`），不打仓库级 tag——Go 工具链只认模块前缀，仓库级版本会制造"所有模块同步发版"的假耦合。
- 大版本进阶（刻意决策，不轻易打）：开发期结束、承诺兼容时升 `v1.0.0`（路径不变，平滑）；仅当 API 整代推倒重来才走 v2+——Go 规定 major≥2 必须改 import 路径 `/vN`（等于换门牌，所有使用方跟着改）。
