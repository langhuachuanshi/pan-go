...
**做**：HTTP 执行器（httpx）、统一 Invoker 接口、统一错误语义（errors）、新网盘接入指南（template 文档）、四模块逐个迁移。

**不做**（显式排除，防 scope 膨胀）：**通用文件模型接口（用户裁决推迟，见决议 6）**、分片上传骨架（二期）、通用重试策略配置化（httpx 只带简单重试）、统一各模块业务 Service 的方法集合、统一各模块的鉴权实现（鉴权是差异，只统一插槽位置）。
...
通用文件模型接口（触发条件：出现第一个真实的跨网盘消费者，届时按真实需求定方法集）。

## 设计原则（本次讨论沉淀）

抽象的入场券：同一段逻辑写了 3 份以上，且差异能用几个参数描述清楚。
- httpx（4 份、差异小）→ 收；errors（4 份、差异=码表）→ 收；invoker 形状（4 份）→ 收
- 文件模型（消费者 0 个）→ 投机，推迟；上传骨架（3 份、差异大）→ 观望
- Go 红利：接口隐式满足，"先用、消费者出现再定义接口"零成本——不为未来赌接口。
